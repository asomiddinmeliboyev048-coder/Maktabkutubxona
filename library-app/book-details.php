...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errors === []) {
        try {
            ...
            if ($lockedBook['listing_type'] === 'sale') throw new RuntimeException('Faqat sotuvdagi kitobni kutubxona band qilish tizimi orqali ijaraga olib bo‘lmaydi.');
            ...
            $pdo->commit();
            set_flash('success', 'Band qilish so‘rovingiz kutubxonaga yuborildi.');
...
$statement = $pdo->prepare(
    "SELECT b.*,c.name category_name,CONCAT_WS(' ',u.first_name,u.last_name) seller_name,
     ...
     GROUP BY b.id,c.name,u.first_name,u.last_name"
);
...
<head>
    ...
    <meta name="description" content="<?= e($book['title']) ?> kitobi haqida ma’lumot">
...
<nav class="navbar navbar-expand-lg navbar-light bg-white sticky-top border-bottom">
    <div class="container py-2">
        ...
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#detailsNavigation" aria-controls="detailsNavigation" aria-expanded="false" aria-label="Menyuni ochish">
            <span class="navbar-toggler-icon"></span>
        </button>
        <div class="collapse navbar-collapse" id="detailsNavigation">
            <div class="ms-auto mt-3 mt-lg-0 d-flex flex-column flex-lg-row align-items-lg-center gap-2">
                <a class="btn btn-outline-primary" href="<?= e(APP_URL) ?>/index.php"><i class="fa-solid fa-table-cells-large me-2"></i>Katalog</a>
        ...
        <div class="container">
            <div class="row align-items-center g-5">
                <div class="col-md-4 col-lg-3">
                    <div class="details-cover-frame">
                        <img class="details-cover" src="<?= e(book_cover_url($book['cover_image'])) ?>" alt="<?= e($book['title']) ?> muqovasi">
                    </div>
                </div>
                <div class="col-md-8 col-lg-9">
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-3 mb-3">
                        <div class="rating-large"><?= render_stars((float) $book['average_rating']) ?></div>
                        <span class="rating-number"><?= number_format((float) $book['average_rating'], 1) ?> · <?= (int) $book['review_count'] ?> ta fikr</span>
                        <span class="listing-badge"><?= e(library_feature_listing_label($book['listing_type'])) ?></span>
                    </div>
                    <div class="availability-summary mb-4">
                        <div class="availability-box status-<?= e($book['availability_status']) ?>">
                            <i class="fa-solid fa-book-open"></i>
                            <div><strong><?= e(library_feature_status_label($book['availability_status'])) ?></strong><span><?= (int) $book['free_copies'] ?> ta erkin nusxa</span></div>
                        </div>
                        <div class="stock-metric"><strong><?= (int) $book['total_copies'] ?></strong><span>Jami</span></div>
            ...
            <div class="col-lg-7">
                <section class="content-card mb-4">
                    ...
                    <dl class="contact-list">
                        <div><dt>Ta’minlovchi</dt><dd><?= e($book['seller_name'] ?: 'Maktab kutubxonasi') ?></dd></div>
                        <div><dt>Manzil</dt><dd><?= e($book['address'] ?: 'Maktab kutubxonasi') ?></dd></div>
                        <div><dt>Telefon</dt><dd><?= e($phone ?: 'Ko‘rsatilmagan') ?></dd></div>
                    </dl>
                    <?php if ($phone !== ''): ?>
                        <div class="d-flex flex-wrap gap-2">
                            <a class="btn btn-primary" href="tel:<?= e($phone) ?>"><i class="fa-solid fa-phone me-2"></i>Qo‘ng‘iroq</a>
                            <?php if ($whatsapp !== ''): ?><a class="btn btn-success" href="https://wa.me/<?= e($whatsapp) ?>" target="_blank" rel="noopener noreferrer"><i class="fa-brands fa-whatsapp me-2"></i>WhatsApp</a><?php endif; ?>
                        </div>
                    <?php endif; ?>
                </section>
                ...
                <section class="content-card">
                    <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
                        <div><p class="section-kicker mb-1">Kitobxonlar</p><h2 class="h4 fw-bold mb-0">Fikrlar</h2></div>
                        <span class="review-count-badge"><?= count($reviews) ?> ta fikr</span>
                    </div>
                    <?php if ($reviews === []): ?><p class="text-secondary mb-0">Bu kitob uchun hali fikr qoldirilmagan.</p><?php endif; ?>
                    <div class="review-list">
                        <?php foreach ($reviews as $review): ?>
                            <article class="review-item">
                                <span class="review-avatar"><i class="fa-solid fa-user"></i></span>
                                <div><?= render_stars((float) $review['rating']) ?><p class="mb-0 mt-2"><?= nl2br(e($review['comment'])) ?></p></div>
                            </article>
                        <?php endforeach; ?>
                    </div>
                </section>
            ...
            <div class="col-lg-5">
                <section class="content-card reservation-card sticky-lg-top">
                    ...
                    <?php if ($book['listing_type'] === 'sale'): ?>
                        <div class="alert alert-info">Bu kitob faqat sotuvda. Ta’minlovchi bilan telefon yoki WhatsApp orqali bog‘laning.</div>
                    <?php elseif (!$user): ?>
                        <div class="alert alert-info">Band qilish uchun o‘quvchi hisobiga kiring.</div>
                        <a class="btn btn-primary w-100" href="<?= e(APP_URL) ?>/login.php">Kirish</a>
                    <?php elseif ($user['role'] !== 'student'): ?>
                        <div class="alert alert-warning">Bu hisob orqali kitob band qilib bo‘lmaydi.</div>
                    ...
                    <?php else: ?>
                        ...
                        <form method="post">
                            ...
                            <label class="form-label" for="pickup_date">Olib ketish sanasi</label>
                            <input class="form-control mb-3" type="date" id="pickup_date" name="pickup_date" min="<?= e($minimum) ?>" max="<?= e($latestValue) ?>" value="<?= e($minimum) ?>" required>
                            <button class="btn btn-primary btn-lg w-100" type="submit">So‘rov yuborish</button>
                        </form>
                    <?php endif; ?>
                </section>
            </div>
...
<footer class="public-footer interactive-footer mt-5" data-interactive-footer>
    ...
    <div class="container py-5 position-relative">
        <div class="row g-4 align-items-center">
            <div class="col-lg-7"><span class="footer-mark"><i class="fa-solid fa-book-open"></i></span><h2 class="h3 text-white mt-3">Har bir sahifa — yangi imkoniyat</h2><p class="mb-0">Kitobni toping, mavjudligini tekshiring va qulay kunga band qiling.</p></div>
            <div class="col-lg-5 text-lg-end"><a class="btn btn-light rounded-pill px-4" href="#detailsNavigation">Yuqoriga <i class="fa-solid fa-arrow-up ms-2"></i></a></div>
        </div>
